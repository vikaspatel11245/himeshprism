...
export function useExerciseTracker() {
  ...
  const startCamera = useCallback(async () => {
    ...
    if (!landmarkerRef.current) {
      alert(
        "AI models are still downloading (about 5MB). Please wait 5-10 seconds and try again!"
      );
      return;
    }
    ...
    // ✅ Let MediaPipe Camera handle permissions and stream entirely
  ...
  const stopCamera = useCallback(() => {
    ...
    // ✅ Properly release the hardware camera stream
